...
Uses Docker and Docker Compose to create a network of services, including the following: app (app/), auth (auth/), play (play/), collab (collab/) and nginx (nginx/).
...
- auth (auth): Right now it's some silly little lottery simulator... Not built with Dockerfile, basic image, volumes, and command specified in root docker-compose.yml... probably will scrap/rework this entirely (found at /auth endpoint).
- play (play): This is a simple functioning http and websocket NodeJS server that can receive http requests and respond or upgrade them to websocket connections, used for the /play endpoint 
- collab (collab): A simple Python WebSocket server using asyncio ... use .venv3.13 for local testing 
- nginx: Provides a simple load-balancing configuration (lb.conf) for receiving requests on port 8080 and routing based on request path to the appropriate service and port (app as /:3000; auth as auth/:6502; play as play/:9090) as configured in the respective directories' sourcecode. The docker-compose.yml file attaches the external port 1313 to the nginx container's 8080, meaning navigating to localhost:1313 hits the Nginx LB and gets routed to the appropriate backend container based on path (app/ by default). 
